        ...
        >
            ...
            <div className="relative z-10 grid min-h-screen grid-cols-1 items-center gap-10 px-[clamp(20px,5vw,52px)] pt-[100px] pb-[80px] md:grid-cols-2 md:gap-16 md:pt-[90px]">
                ...
                <div className="flex flex-col">
                    ...
                    >
                        <div className="h-[7px] w-[7px] rounded-full bg-[#22d3ee] pulse-dot" />
                    ...
                    <motion.div {...up(0.58)}>
                        ...
                        >
                            ...
                            <span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(135deg, #22d3ee, #38bdf8)" }}>{w2}</span>
                    ...
                    {/* Description */}
                    <motion.p {...up(0.75)} className="mb-7 max-w-[460px] text-[clamp(13px,1.4vw,15px)] leading-[1.9] text-slate-400">
                        A passionate{" "}
                    ...
                    <motion.div {...up(0.9)} className="mb-9 flex flex-wrap gap-3">
                        ...
                        >
                            ...
                            <svg className="relative z-10 shrink-0" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3">
                        ...
                        >
                            ...
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className="shrink-0 transition-transform duration-200 group-hover:translate-y-[3px]">
                        ...
                        {STATS.map((s) => (
                            <div key={s.label}>
                                <div
                                    ...
                                    style={{ fontFamily: "var(--font-syne)", color: s.cyan ? "#22d3ee" : "#e2e8f0" }}
                                >
                                    {s.val}
                                </div>
                ...
                >
                    <div className="relative flex flex-col items-center gap-5">
                        <div className="relative h-[260px] w-[260px] md:h-[300px] md:w-[300px]">
                            {/* Spinning rings */}
                            <div className="spin-slow absolute inset-[-18px] rounded-full" style={{ border: "1.5px dashed rgba(34,211,238,0.18)" }} />
                            <div className="spin-rev absolute inset-[-40px] rounded-full" style={{ border: "0.5px solid rgba(34,211,238,0.06)" }} />

                            {/* Photo */}
                            <div className="relative h-full w-full overflow-hidden rounded-full" style={{ border: "1.5px solid rgba(34,211,238,0.2)" }}>
                                <div className="absolute top-0 left-0 right-0 h-[2px] z-10" style={{ background: "linear-gradient(90deg, #22d3ee, #38bdf8, transparent)" }} />
                                <Image src="/opi.jpg" alt="Md Opi Korim — Full Stack Developer" fill className="object-cover object-top" priority />
                            </div>
                        ...
                        {/* Name tag */}
                        <div className="rounded-[10px] px-5 py-[10px] text-center" style={{ background: "rgba(10,22,44,0.7)", border: "1px solid rgba(34,211,238,0.1)", backdropFilter: "blur(8px)" }}>
                            <div className="text-[15px] font-bold text-slate-200" style={{ fontFamily: "var(--font-syne)" }}>Md Opi Korim</div>
                            <div className="mt-[2px] text-[11px] tracking-[0.07em]" style={{ background: "linear-gradient(135deg, #22d3ee, #38bdf8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                                Full Stack Developer
                            </div>
                        </div>
                    </div>
            ...
            {/* Scroll hint */}
            <motion.div {...fadeIn(1.4)} className="absolute bottom-6 left-1/2 flex -translate-x-1/2 flex-col items-center gap-[6px]">
                <span className="text-[10px] font-mono uppercase tracking-[0.18em]" style={{ color: "rgba(34,211,238,0.3)" }}>scroll</span>
                <div className="flex flex-col items-center">
                    {[0, 1, 2].map((n) => (
                        <svg key={n} width="14" height="9" viewBox="0 0 14 9" fill="none"
...
function OrbitIcon({ ic, pos, desktop = false }: { ic: typeof ORBIT[number]; pos: React.CSSProperties; desktop?: boolean }) {
    return (
        <div
            title={ic.label}
            className={["absolute flex h-[46px] w-[46px] items-center justify-center rounded-[12px]", desktop ? "hidden md:flex" : "flex"].join(" ")}
            style={{ ...pos, background: ic.bg, border: `1px solid ${ic.border}`, backdropFilter: "blur(6px)", animation: "floatA 3.4s ease-in-out infinite" }}
        >
            {ic.icon}
        </div>
    );
